...
async function getExpenses(req, res, next) {
    try {
        const results = await Expense.find({ 'user.user_id': req.user.user_id }, { description: 0 });
        res.json(results);
    } catch (error) {
        next(error);
    }
...
async function getExpensesByDate(req, res, next) {
    try {
        const results = await Expense.find({ 'user.user_id': req.user.user_id },{'date' : req.body.date});
        res.json(results);
    } catch (error) {
        next(error);
    }
...
async function updateExpenseById(req, res, next) {
    try {
        const results = await Expense.updateOne({ _id: req.params.expense_id },
            { $set: { ...req.body } });
        res.json(results);
    } catch (error) {
        next(error);
