...
@Component({
  selector: 'app-todo',
  template: `
      <div [setBackground]="todo.completed? 'grey': ''">{{todo.title}} - {{todo.timestamp | date:'short'}}</div>
      ...
      <button (click)="actions.updateCompleted($any(todo)._id, false)" *ngIf="todo.completed">Mark Incomplete</button>
  `
})
